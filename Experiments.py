    ...
    def run_all_experiments(self, iterations):
        ...
        for iter_id in range(iterations):
            ...
            np_test_X, np_test_T, np_test_yf, np_test_ycf, n_treated, n_total = \
                self.dL.load_train_test_lalonde_random(csv_path,
                                                     split_size)
            ...
            y1_hat_orig = self.dL.inverse_transform_outcomes(dr_eval_out["y1_hat_list"])
            y0_hat_orig = self.dL.inverse_transform_outcomes(dr_eval_out["y0_hat_list"])
            y1_true_orig = self.dL.inverse_transform_outcomes(dr_eval_out["y1_true_list"])
            y0_true_orig = self.dL.inverse_transform_outcomes(dr_eval_out["y0_true_list"])
            ...
            print("drnet_PEHE_out (in dollars): ", drnet_PEHE_out)
            print("drnet_ATE_metric_out (in dollars): ", drnet_ATE_metric_out)

            dr_eval_in = drnet_manager.test_DR_NET({"tensor_dataset": tensor_train}, device)
            print("---" * 20)

            # Transform training predictions back to original scale
            y1_hat_orig = self.dL.inverse_transform_outcomes(dr_eval_in["y1_hat_list"])
            y0_hat_orig = self.dL.inverse_transform_outcomes(dr_eval_in["y0_hat_list"])
            y1_true_orig = self.dL.inverse_transform_outcomes(dr_eval_in["y1_true_list"])
            y0_true_orig = self.dL.inverse_transform_outcomes(dr_eval_in["y0_true_list"])
            ...
            print("drnet_PEHE_in (in dollars): ", drnet_PEHE_in)
            print("drnet_ATE_metric_in (in dollars): ", drnet_ATE_metric_in)
            ...
            file1.write("\nToday's date: {0}\n".format(date.today()))
            file1.write("Iter: {0}, PEHE_DR_NET_out: ${1:.2f}, ATE_DR_NET_out: ${2:.2f}, "
                        "PEHE_DR_NET_in: ${3:.2f}, ATE_DR_NET_in: ${4:.2f}, \n"
