...
def perform_matching(df_encoded):
    start_time = time.time()
    treated_pscores = df_encoded[df_encoded[TREATMENT_VAR] == 1]['pscore'].values.reshape(-1, 1).astype('float32')
    control_pscores = df_encoded[df_encoded[TREATMENT_VAR] == 0]['pscore'].values.reshape(-1, 1).astype('float32')
    ...
    k = min(N_NEIGHBORS, len(df_encoded[df_encoded[TREATMENT_VAR] == 0]))
    distances, indices = index.search(treated_pscores, k)
    print(f"FAISS matching time: {time.time() - start_time:.2f} seconds")
    return distances, indices

def find_matches(df_encoded, distances, indices, caliper, with_replacement=False):
    start_time = time.time()
    matched_pairs = []
    ...
    treatment_idx_by_ps = df_encoded[df_encoded[TREATMENT_VAR] == 1]['pscore'].sort_values().index

    for treated_idx in treatment_idx_by_ps:
        i = df_encoded[df_encoded[TREATMENT_VAR] == 1].index.get_loc(treated_idx)
        matches = indices[i]
        treated_pscore = df_encoded.loc[treated_idx, 'pscore']
        valid_matches = []

        for control_idx in matches:
            if control_idx == -1:
                continue
            control_pscore = df_encoded.iloc[control_idx]['pscore']
        ...
        if len(valid_matches) == MATCHING_RATIO:
            matching_map[treated_idx] = [df_encoded.index[m] for m in valid_matches]
        else:
            unmatched_treated += 1

    for treated_idx, control_indices in matching_map.items():
        treated_record = df_encoded.loc[treated_idx].copy()
...
def run_analysis(df):
    ...
    df_encoded['pscore'] = calculate_propensity_scores(df_encoded, features_for_model)

    # Perform matching
    caliper = np.std(df_encoded['pscore']) * CALIPER_SD
    distances, indices = perform_matching(df_encoded)
    matched_pairs, unmatched_treated, matching_map = find_matches(df_encoded, distances, indices, caliper, with_replacement=True)
...
def psm_function(df, exact_match=[]):
    ...
    # Preprocess and engineer features for the entire dataset
    df_clean = preprocess_data(df)
    df_encoded, categorical_cols = create_dummy_variables(df_clean)

    # Perform matching on the entire dataset
    matched_pairs, matching_map = run_analysis(df_encoded)

    # Filter the matched pairs based on exact matching criteria
    if exact_match:
        matched_pairs = matched_pairs[matched_pairs[exact_match].duplicated(keep=False)]

    print(f"Total PSM function execution time: {time.time() - start_time:.2f} seconds")
    return matched_pairs, matching_map
...
print(f"Total program execution time: {time.time() - start_time:.2f} seconds")
