...
class Utils:
    @staticmethod
    def preprocess_lalonde_features(data_df):
        """Preprocess LaLonde dataset features"""
        # Separate numerical and categorical columns
        num_cols = ['age', 'educ', 're74', 're75']
        cat_cols = ['race']
        ...
        race_encoded = encoder.fit_transform(data_df[cat_cols])
        race_cols = [f'race_{i}' for i in range(race_encoded.shape[1])]
        
        # Combine processed features
        processed_df = data_df.copy()
        processed_df[race_cols] = race_encoded
        processed_df.drop('race', axis=1, inplace=True)
    ...
    def convert_to_tensor_DR_net(X, T, Y_f, Y_cf, pi, mu):
        tensor_x = torch.FloatTensor(X)  # Modified to handle preprocessed features directly
        tensor_T = torch.FloatTensor(T)
        tensor_y_f = torch.FloatTensor(Y_f)
        tensor_y_cf = torch.FloatTensor(Y_cf)
        tensor_pi = torch.FloatTensor(pi)
    ...
    def convert_to_tensor_mu_net(X, T, yf):
        tensor_x = torch.FloatTensor(X)  # Modified to handle preprocessed features directly
        tensor_T = torch.FloatTensor(T)
        tensor_y_f = torch.FloatTensor(yf)
...
class EarlyStopping_DCN:
    ...
    def __init__(self, patience=7, verbose=False, delta=0,
                 dr_net_shared_path="Lalonde/Models/dr_net_phi.pth",
                 dr_net_y1_path="Lalonde/Models/dr_net_y1.pth",
                 dr_net_y0_path="Lalonde/Models/dr_net_y0.pth",
                 pi_net_path="Lalonde/Models/pi_net.pth",
                 mu_net_path="Lalonde/Models/mu_net.pth",
                 trace_func=print):
        """
        Args:
